...
def test_telegram_endpoint_requires_profile_token_and_allowed_identity(
    ...
    assert accepted_payload["proposal"]["proposal_id"] == proposal["proposal_id"]
    assert accepted_payload["proposal_status"] == "proposed"
    assert accepted_payload["approval_status"] == "pending"
    assert accepted_payload["requires_approval"] is True
    assert accepted_payload["applied"] is False


def test_telegram_reject_reports_rejected_not_applied(
    config,
    monkeypatch,
    tmp_path: Path,
):
    ...
    with TestClient(mcp.http_app(path="/mcp")) as client:
        response = client.post(
            ...
            json={
                "action": "reject",
                "proposal_id": proposal["proposal_id"],
                "proposal_sha256": proposal["diff_sha256"].removeprefix("sha256:"),
                "telegram_user_id": "123",
            },
            headers={"Authorization": f"Bearer {token}"},
        )
...
def test_telegram_approval_delivers_and_cleans_managed_worktree(
    ...
    assert payload["proposal_status"] == "applied"
    assert payload["approval_status"] == "applied"
    assert payload["requires_approval"] is False
    assert payload["applied"] is True
    assert payload["cleanup"]["ok"]
    assert (repository / "app.py").read_text() == "delivered\n"
    assert not job.worktree.exists()
    assert repeated.status_code == 200
    repeated_payload = repeated.json()
    assert repeated_payload["already_delivered"]
    assert repeated_payload["proposal_status"] == "applied"
    assert repeated_payload["applied"] is True
