...
`check_execution_health.ok=false`이면 개발 실행으로 넘어가지 않는다. 응답의
`recommended_next_action`을 보고 runtime registration, cwd mapping, sandbox, worktree
문제를 먼저 해결한다.
...
MCP HTTP stream 문제와 서버 생존 상태를 분리하기 위해 일반 HTTP endpoint를 제공한다.

- `GET /healthz`: 서버 생존, MCP tool count, configured profiles, registered runtime profiles
- `GET /readyz`: `/healthz` 정보에 auth/artifact/worktree readiness 정보 추가

Hermes에서 `ClosedResourceError`가 발생해도 `/healthz`가 `ok=true`이면
host-coding-agent 서버 다운이 아니라 Hermes MCP HTTP stream client/reconnect 문제로
분리해서 진단한다.
...
`report` 작업은 approval을 만들지 않고 immutable proposal만 생성한다. 원본 workspace,
commit, PR은 만들지 않으며 조사/검토를 위해 worktree를 보존한다.
...
profiles:
  dev-bot:
    allowed_delivery_modes: ["manual", "report", "auto", "commit", "pr"]
...
Direct는 원본을 즉시 수정하므로 적용 승인과 자동 rollback이 없다. Direct 응답에는
`changed_files`, `changed_file_count`, `applied_immediately`가 포함된다. 사용자가
"확인만", "분석만" 같은 read-only 의도로 요청했는데 direct 경로를 써야 한다면
`direct_write_policy`를 `fail_if_changed`로 지정한다. 이 경우 변경이 발생하면
`ok=false`, `error_code=direct_write_policy_violation`으로 반환한다. 이미 발생한 direct
변경을 자동 rollback하지는 않는다. 변경 전 검토, 실패 격리, 승인 적용이 필요하면
다음처럼 worktree를 요청한다.

`delivery_mode`는 worktree delivery에만 적용된다.

| isolation_mode | delivery_mode | 결과 |
|---|---|---|
| direct | manual | 유효. 원본 workspace를 즉시 수정 |
| direct | commit/auto/pr/report | 무효. `invalid_isolation_delivery_combination` |
| worktree | manual | 유효. proposal 생성 후 외부 승인 대기 |
| worktree | report | 유효. proposal만 생성하고 apply/commit/PR 없이 worktree 보존 |
| worktree | commit/auto/pr | profile 권한이 허용하면 유효 |
...
5. `deliver_development_job`: commit/auto/PR delivery 실행. Manual은 approval 정보,
   report는 적용 없는 proposal report 반환
