...
장기 개발은 `start_development_task`가 즉시 profile-scoped `job_id`를 반환하고
SQLite에 상태·단계·결과·구조화 이벤트를 저장한다. `get_async_job_events`는 sequence
cursor로 새 이벤트만 조회한다.
...
- `run_development_task` 단일 MCP workflow API 구현
- 단일 호출 manual approval 대기와 commit delivery E2E 검증 구현
- 단계별 실패 위치·job 상태를 반환하고 예외 시 repository lock 해제 구현
- profile별 `direct`/`worktree` 허용 및 기본 isolation mode 설정 구현
- direct mode의 non-Git 원본 workspace write sandbox 실행 구현
- direct mode는 worktree/proposal/commit 없이 즉시 반영하도록 단일 API 분기 구현
...
- 대용량 비동기 job result 자체는 아직 pagination하지 않으며 이벤트만 cursor pagination한다.

## 5. 다음 작업

1. Origin/Host 검증과 profile별 동시 실행/queue 제한
2. 대용량 job result pagination 또는 외부 artifact 저장
